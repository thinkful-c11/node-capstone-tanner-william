...
app.post('/tags/evaluate', (req, res)=>{
  ...
  const url1 = `/tags/${type}`;
  const url2 = `/tags/${type}/${tag}`;
  Stubs
    .find({tag})
    .count()
    .then(count => {
      if(count > 0){
        fetch(url1, {method: 'PUT', body: data, 'content-type': 'application/json'});
      }else {
        fetch(url2, {method: 'POST', body: data, 'content-type': 'application/json'})
        .then(_res => console.log(_res)).catch(err => console.error(err));
      }
    });
});

app.post('/tags/:type/:tag', (req, res)=>{
